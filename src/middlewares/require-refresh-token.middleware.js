...
export const validateRefreshToken = (usersRepository, refreshTokenRepository) => {
    return async (req, res, next) => {
        try {
            ...
            const exitedRefreshToken = await refreshTokenRepository.findRefreshToken(decodedToken.id);
            //넘겨받은 refreshToken과 비교(optional chaining 사용)
            const isValidRefreshToken =
                exitedRefreshToken &&
                exitedRefreshToken?.token &&
                bcrypt.compareSync(refreshToken, exitedRefreshToken.token);
